...
public class NewConvertsYearActivity extends ActionBarActivity {
	
	private PreferenceHelper mPreferenceHelper;
	ListView Newconvertlistview;
	String year;
	private Gson gson;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.newconverts_year);
		Newconvertlistview=(ListView) findViewById(R.id.yearlist);
	...
	//	filterimg=(ImageView) findViewById(R.id.imageView2);
		...
		fromDatePickerDialog = new DatePickerDialog(NewConvertsYearActivity.this, new OnDateSetListener() {
		...
		toDatePickerDialog = new DatePickerDialog(NewConvertsYearActivity.this, new OnDateSetListener() {
		...
		/*filterimg.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(NetworkHelper.isOnline(NewConvertsYearActivity.this)){
					
						showDialog();
					
					}else{
					
						Methods.longToast("Please connect to Internet",NewConvertsYearActivity.this);
		...
		Newconvertlistview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				try {
					...
					Intent it=new Intent(NewConvertsYearActivity.this, DashboardListDetailsConvertsActivity.class);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_addTo:
			...
			Intent intCreateEvent=new Intent(NewConvertsYearActivity.this,CreateEventActivity.class);
			startActivity(intCreateEvent);
			
			break;

		case R.id.menu_option2:
			Intent intMyEventList=new Intent(NewConvertsYearActivity.this,EventListActivity.class);
	...
	private void getDashboardDataService(final String pageno) {
		StringRequest reqDashboard=new StringRequest(Method.POST,DashboardListDataService.SERVICE_URL,new Listener<String>() {
			...
			public void onResponse(String response) {
				...
				Log.e("droid","get all New converts Year ---------------"+ response);
				...
				//	filterimg.setVisibility(View.GONE);
					...
					if(respModel.getMessage().getStatus()=="401"){
						Methods.longToast("User name or Password is incorrect", NewConvertsYearActivity.this);
					}else{
						Methods.longToast(respModel.getMessage().getMessage(), NewConvertsYearActivity.this);
					}
				}else{
					...
					try {
						...
						if(jsonarray.length()>0){ 
							...
							EventListAdapter adpter=new EventListAdapter(NewConvertsYearActivity.this,jsonarray);
							Newconvertlistview.setAdapter(adpter);
							
						}else{
							//filterimg.setVisibility(View.GONE);
							Methods.longToast("No results found", NewConvertsYearActivity.this);
						}
	        ...
	        btns[j].setOnClickListener(new OnClickListener() {
	            ...
	            public void onClick(View v) 
	            {
	               	if(NetworkHelper.isOnline(NewConvertsYearActivity.this)){
	               		...
	               		Methods.showProgressDialog(NewConvertsYearActivity.this);
...
public void showDialog(){
		...
		LayoutInflater layoutInflater = LayoutInflater.from(NewConvertsYearActivity.this);
		View promptView = layoutInflater.inflate(R.layout.filter_dialog, null);
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(NewConvertsYearActivity.this);
				...
				if(SpinnerDataFlag.Zones_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Zonal Pastor")){
						...
						try{
							...
							Toast.makeText(NewConvertsYearActivity.this, "Please Select Resion", Toast.LENGTH_LONG).show();
				...
				if(SpinnerDataFlag.GroupChurches_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Group Church Pastor")){
						...
						try{
							...
							Toast.makeText(NewConvertsYearActivity.this, "Please Select Zone", Toast.LENGTH_LONG).show();
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Church Pastor")){
						...
						try{
							...
							if(spgroupchurchTextView.getVisibility()!=View.VISIBLE){
								...
								Toast.makeText(NewConvertsYearActivity.this, "Please Select Group Church", Toast.LENGTH_LONG).show();
							}
						}catch(NullPointerException ex){
				...
				if(SpinnerDataFlag.PCFs_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("PCF Leader")){
						...
						try{
							...
							Toast.makeText(NewConvertsYearActivity.this, "Please Select Church", Toast.LENGTH_LONG).show();
				...
				if(SpinnerDataFlag.SeniorCells_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Senior Cell Leader")){
						...
						try{
							...
							Toast.makeText(NewConvertsYearActivity.this, "Please Select Pcf", Toast.LENGTH_LONG).show();
				...
				if(SpinnerDataFlag.Cells_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Cell Leader")){
						...
						try{
							...
							Toast.makeText(NewConvertsYearActivity.this, "Please Select Senior cell", Toast.LENGTH_LONG).show();
		...
		public void onClick(DialogInterface dialog, int id) {
			...
			try{
			 ...
			 if(!checkValidation(resion,zone,groupchurch,church,pcf,srcell,cell,fdate,tdate)){
				 ...
				 Methods.longToast("Please select any Filter",NewConvertsYearActivity.this);
			...
			if(NetworkHelper.isOnline(NewConvertsYearActivity.this)){
				Methods.showProgressDialog(NewConvertsYearActivity.this);
				getUpdatedListMethod("New Converts",resion,zone,groupchurch,church,pcf,srcell,cell,fdate,tdate);
	...
	StringRequest reqgetTopHierarchy=new StringRequest(Method.POST,LowerHierarchyService.SERVICE_URL,new Listener<String>() {
		...
		public void onResponse(String response) {
			...
			try {
				...
				if(tblname.equals("Zones")){
					...
					ArrayAdapter<String> adapterchurchgropu = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mGrpChurchList);
				...
				if(tblname.equals("Group Churches")){
					...
					ArrayAdapter<String> adapterChurch = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mChurchList);
				...
				if(tblname.equals("Churches")){
					...
					ArrayAdapter<String> adaptercell = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mCellList);
				...
				if(tblname.equals("PCFs")){
					...
					ArrayAdapter<String> adapterSrCell = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mSeniorCellList);
		...
		public void onErrorResponse(VolleyError error) {
			...
			if(error.networkResponse.statusCode==403){
				Methods.longToast("Access Denied", NewConvertsYearActivity.this);
			}
			else
...
private void setAdapters() {
	...
	ArrayAdapter<String> adapterZone = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mZoneList);
	adapterZone.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
	spzone.setAdapter(adapterZone);

	ArrayAdapter<String> adapterRegion = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mRegionList);
	...
	ArrayAdapter<String> adapterPCF = new ArrayAdapter<String>(NewConvertsYearActivity.this, android.R.layout.simple_spinner_item, mPCFList);
			...
			if(respModel.getMessage().getStatus()=="401"){
				...
				try {
					...
					//if(pageflag)
					 ...
					 titletextView.setText("New Converts("+TOTAL_LIST_ITEMS+")");
				
					if(jsonarray.length()>0){
						
						EventListAdapter adapter=new EventListAdapter(NewConvertsYearActivity.this,jsonarray);
						Newconvertlistview.setAdapter(adapter);
