...
import com.mutech.synergy.activities.meeting.MyMeetingListActivity.Holder;
import com.mutech.synergy.activities.meeting.MyMeetingListActivity.MeetingListAdapter;
...
public class MeetingListActivity extends ActionBarActivity implements OnItemClickListener{
...
//		setContentView(R.layout.new_eventlist_layout);
	...
	private void initialize() {
		...
		fromDatePickerDialog = new DatePickerDialog(MeetingListActivity.this, new OnDateSetListener() {
		...
		toDatePickerDialog = new DatePickerDialog(MeetingListActivity.this, new OnDateSetListener() {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_addTo:
			...
			Intent intCreateEvent=new Intent(MeetingListActivity.this,CreateEventActivity.class);
			startActivity(intCreateEvent);
			
			break;

		case R.id.menu_option2:
			Intent intMyEventList=new Intent(MeetingListActivity.this,MeetingListActivity.class);
	...
	private void getMeetingList(final String pageno) {
		StringRequest reqMeetingList=new StringRequest(Method.POST,GetAllMeetingService.SERVICE_URL,new Listener<String>() {
			...
			public void onResponse(String response) {
				...
				if(response.contains("status"))
				{
					...
					try {
						...
						tvTitle.setText("All Meetings("+TOTAL_LIST_ITEMS+")   ");
						...
						if(jsonarray.length() > 0){
							 ...
							 Btnfooter();
							mMeetingListAdapter=new MeetingListAdapter(MeetingListActivity.this,jsonarray);
							lvMeeting.setAdapter(mMeetingListAdapter);
						}
						else{
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
			long id) {
		Intent i=new Intent(this,SingleMeetingDetails.class);
		Bundle b=new Bundle();
		try {
			b.putString("Meeting_name",jsonarray.getJSONObject(position).getString("meeting_name").toString());
			b.putString("MeetingSubj",jsonarray.getJSONObject(position).getString("meeting_subject").toString());
			b.putString("Meeting_venue",jsonarray.getJSONObject(position).getString("venue").toString());
			b.putString("Meeting_dateTime",jsonarray.getJSONObject(position).getString("meeting_date").toString());
			i.putExtra("Bundle",b);
			startActivity(i);
	...
	class MeetingListAdapter extends BaseAdapter{
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			try {
				...
				if(null !=jsonList.getJSONObject(position).getString("meeting_date").toString())
					holder.lblMeetingTime.setText(jsonList.getJSONObject(position).getString("meeting_date").toString());
			...
			holder.btnMarkAttendance.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					try {
						...
						Intent i=new Intent(MeetingListActivity.this,MarkMemberAttendanceActivity.class);
						Bundle b=new Bundle();
						b.putString("Meeting_name",jsonList.getJSONObject(position).getString("meeting_name").toString());
						b.putString("MeetingSubj",jsonList.getJSONObject(position).getString("meeting_subject").toString());
						b.putString("Meetingtime",jsonList.getJSONObject(position).getString("meeting_date").toString());
						i.putExtra("Bundle",b);
						startActivity(i);
	        ...
	        btns[j].setOnClickListener(new OnClickListener() {
	            ...
	            public void onClick(View v) 
	            {
	               	if(NetworkHelper.isOnline(MeetingListActivity.this)){
	        			...
	        			getMeetingList( btns[j].getText().toString());
	        			
	        		}else{
	        			
	        			Methods.longToast("Please connect to Internet", MeetingListActivity.this);
...
public void showDialog(){
		...
		ArrayAdapter<String> adapterspeventtype = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, meventtype);
				...
				if(SpinnerDataFlag.Zones_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Zonal Pastor")){
						...
						try{
							...
							Toast.makeText(MeetingListActivity.this, "Please Select Resion", Toast.LENGTH_LONG).show();
				...
				if(SpinnerDataFlag.GroupChurches_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Group Church Pastor")){
						...
						try{
							...
							Toast.makeText(MeetingListActivity.this, "Please Select Zone", Toast.LENGTH_LONG).show();
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Church Pastor")){
						...
						try{
							...
							if(spgroupchurchTextView.getVisibility()!=View.VISIBLE){
								...
								Toast.makeText(MeetingListActivity.this, "Please Select Group Church", Toast.LENGTH_LONG).show();
							}
						}catch(NullPointerException ex){
				...
				if(SpinnerDataFlag.PCFs_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("PCF Leader")){
						...
						try{
							...
							Toast.makeText(MeetingListActivity.this, "Please Select Church", Toast.LENGTH_LONG).show();
				...
				if(SpinnerDataFlag.SeniorCells_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Senior Cell Leader")){
						...
						try{
							...
							Toast.makeText(MeetingListActivity.this, "Please Select Pcf", Toast.LENGTH_LONG).show();
				...
				if(SpinnerDataFlag.Cells_flag){
					...
					if(mPreferenceHelper.getString(Commons.USER_ROLE).equals("Cell Leader")){
						...
						try{
							...
							Toast.makeText(MeetingListActivity.this, "Please Select Senior cell", Toast.LENGTH_LONG).show();
	...
	StringRequest reqgetTopHierarchy=new StringRequest(Method.POST,LowerHierarchyService.SERVICE_URL,new Listener<String>() {
		...
		public void onResponse(String response) {
			...
			try {
				...
				if(tblname.equals("Zones")){
					...
					ArrayAdapter<String> adapterchurchgropu = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mGrpChurchList);
				...
				if(tblname.equals("Group Churches")){
					...
					ArrayAdapter<String> adapterChurch = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mChurchList);
				...
				if(tblname.equals("Churches")){
					...
					ArrayAdapter<String> adaptercell = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mCellList);
				...
				if(tblname.equals("PCFs")){
					...
					ArrayAdapter<String> adapterSrCell = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mSeniorCellList);
		...
		public void onErrorResponse(VolleyError error) {
			...
			if(error.networkResponse.statusCode==403){
				Methods.longToast("Access Denied", MeetingListActivity.this);
			}
			else
...
private void setAdapters() {
	...
	ArrayAdapter<String> adapterZone = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mZoneList);
	adapterZone.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
	spzone.setAdapter(adapterZone);

	ArrayAdapter<String> adapterRegion = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mRegionList);
	...
	ArrayAdapter<String> adapterPCF = new ArrayAdapter<String>(MeetingListActivity.this, android.R.layout.simple_spinner_item, mPCFList);
				...
				try {
					...
					if(jsonarray.length()>0){
						...
						mMeetingListAdapter.notifyDataSetChanged();
							...
							Methods.longToast("No results found", MeetingListActivity.this);
							mMeetingListAdapter=new MeetingListAdapter(MeetingListActivity.this,jsonarray);
							lvMeeting.setAdapter(mMeetingListAdapter);
							mMeetingListAdapter.notifyDataSetChanged();
					}
